...
export async function onRequestPost({request, env}) {
  ...
  const credRow = await env.DB.prepare('SELECT * FROM bio_credentials WHERE cred_id = ?').bind(credential.id).first();
  if (!credRow) return json({ error: 'Credencial no reconocida' }, 401);
  ...
  await env.DB.prepare('UPDATE bio_credentials SET counter = counter + 1 WHERE cred_id = ?').bind(credential.id).run();
